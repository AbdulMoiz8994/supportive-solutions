                ...
                <div class="mx-auto flex w-full max-w-md flex-1 flex-col justify-center">
                    <div class="mb-5 sm:mb-8">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            A verification code has been sent to your {{ session('2fa_method') ?? 'device' }}. Please enter
                            it in the field below.
                        </p>
                    </div>
                    <div x-data="{ loading: false }">
                        <form action="{{ route('two-factor.verify') }}" method="POST" id="otp-form" @submit="loading = true">
                            @csrf
                            ...
                            <div class="space-y-5">
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-2 sm:gap-4" id="otp-container">
                                        <input type="text" maxlength="1" class="dark:bg-dark-900 otp-input shadow-theme-xs ... h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-center text-xl font-semibold text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" required />
                                        <input type="text" maxlength="1" class="dark:bg-dark-900 otp-input shadow-theme-xs ... h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-center text-xl font-semibold text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" required />
                                        <input type="text" maxlength="1" class="dark:bg-dark-900 otp-input shadow-theme-xs ... h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-center text-xl font-semibold text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" required />
                                        <input type="text" maxlength="1" class="dark:bg-dark-900 otp-input shadow-theme-xs ... h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-center text-xl font-semibold text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" required />
                                        <input type="text" maxlength="1" class="dark:bg-dark-900 otp-input shadow-theme-xs ... h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-center text-xl font-semibold text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" required />
                                    ...
                                    @if(config('app.debug') && session('debug_otp'))
                                        <div class="mt-4 p-3 rounded-lg bg-blue-50 border border-blue-200 text-blue-700 text-sm">
                                            <p class="font-bold flex items-center gap-2">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                                Debug Mode: OTP Code
                                            </p>
                                            <p class="mt-1 font-mono text-lg tracking-widest">{{ session('debug_otp') }}</p>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div>
                                    <button
                                        ...
                                        <span x-text="loading ? 'Verifying...' : 'Verify My Account'"></span>
                                    </button>
                                </div>
                            </div>
                        </form>
                        <div class="mt-5">
                            <p class="text-center text-sm font-normal text-gray-700 sm:text-start dark:text-gray-400">
                                Code expires in: <span id="otp-timer" class="font-bold text-brand-500">30:00</span>
                            </p>
                            <p class="mt-2 text-center text-sm font-normal text-gray-700 sm:text-start dark:text-gray-400">
                                Didn’t get the code?
                                <form action="{{ route('two-factor.resend') }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" id="resend-btn" class="text-brand-500 hover:text-brand-600 dark:text-brand-400 font-medium opacity-50 cursor-not-allowed" disabled>Resend</button>
                                </form>
                            </p>
                        </div>
                    </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Timer Logic
        const expiryTime = {{ auth()->user()->two_factor_expires_at->timestamp }} * 1000;
